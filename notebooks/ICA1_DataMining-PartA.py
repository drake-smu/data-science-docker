...
# In the videos, we derived the formula for calculating the optimal values of
# the regression weights (you must be connected to the internet for this
# equation to show up properly):
#
# $$ w = (X^TX)^{-1}X^Ty $$
#
# where $X$ is the matrix of values with a bias column of ones appended onto it.
# For the diabetes dataset one could construct this $X$ matrix by stacking a
# column of ones onto the `ds.data` matrix. 
#
# $$ 
# X=\begin{bmatrix} & \vdots &        &  1 \\
#         \dotsb & \text{ds.data} & \dotsb &  \vdots\\
#          & \vdots &         &  1\\
#      \end{bmatrix}
# $$
#
# **Question 1:** For the diabetes dataset, how many elements will the vector
# $w$ contain?
...
# ________________________________________________________________________________________________________
#
# **Exercise 1:** In the following empty cell, use this equation and numpy
# matrix operations to find the values of the vector $w$. You will need to be
# sure $X$ and $y$ are created like the instructor talked about in the video.
# Don't forget to include any modifications to $X$ to account for the bias term
# in $w$. You might be interested in the following functions:
#
# - `np.hstack((mat1,mat2))` stack two matrices horizontally, to create a new
#   matrix
# - `np.ones((rows,cols))` create a matrix full of ones
# - `my_mat.T` takes transpose of numpy matrix named `my_mat`
# - `np.dot(mat1,mat2)` or `mat1 @ mat2` is matrix multiplication for two
#   matrices
# - `np.linalg.inv(mat)` gets the inverse of the variable `mat`

#%%
# Write you code here, print the values of the regression weights using the
